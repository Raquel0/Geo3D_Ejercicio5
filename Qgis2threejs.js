...
Q3D.Scene.prototype.loadJSONObject = function(jsonObject) {
    if (jsonObject.type == "scene") {
        ...
        if (jsonObject.properties !== undefined) {
            ...
            this.userData.origin = {
                x: this.userData.baseExtent[0] + w / 2,
...
Q3D.Scene.prototype.toMapCoordinates = function(x, y, z) {
    ...
    return {
        x: x / this.userData.scale + this.userData.origin.x,
...
(function() {
    ...
    app.init = function(container) {
        ...
        if (typeof THREE.OrbitControls !== "undefined") {
            ...
            effect = new THREE.AnaglyphEffect(app.renderer);
    ...
    app.intersectObjects = function(offsetX, offsetY) {
        var vec2 = new THREE.Vector2((offsetX / app.width) * 2 - 1, -(offsetY / app.height) * 2 + 1);
        ...
        if (app.labelVisible) {
            ...
            for (var i = 0; i < rootGroup.children.length; i++) {
                ...
                for (var k = 0; k < connGroup.children.length; k++) {
                    ...
                    labels.push({
                        pt: new THREE.Vector3(pt.x, pt.y, pt.z), // in world coordinates
...
Q3D.DEMBlock.prototype = {
    ...
    buildFrame: function(layer, parent, material, z0) {
        ...
        v = [
            [-hw, -hh, grid.array[grid.array.length - grid.width]],
...
Q3D.LineLayer.prototype.build = function(features) {
    ...
    if (objType == this._lastObjType && this._createObject !== undefined) {
        ...
        var mesh, pt0 = new THREE.Vector3(),
        ...
        vi = [
            [0, 5, 4],
            [4, 5, 1], // left turn - top, side, bottom
            [3, 0, 7],
            [7, 0, 4],
            [6, 3, 2],
            [2, 3, 7],
            [4, 1, 0],
            [0, 1, 5], // right turn - top, side, bottom
            [1, 2, 5],
            [5, 2, 6],
            [2, 7, 6],
            [6, 7, 3]
...
loader.load(
    ...
    function(gltf, mesh) {
        ...
        Q3D.application.scene.add(gltf.scene);
